...
"""
baseline_data = utils.get_class_from_data(all_subj_pkl, 'baseline')
print("length of baseline_data : {} of type {}".format(len(baseline_data), type(baseline_data)))
stress_data = utils.get_class_from_data(all_subj_pkl, 'stress')
print("length of stress_data : {}".format(len(stress_data)))
amusement_data = utils.get_class_from_data(all_subj_pkl, 'amusement')
print("length of amusement_data : {}".format(len(amusement_data)))
meditation_data = utils.get_class_from_data(all_subj_pkl, 'meditation')
...
baseline_data_train_clean = utils.df_to_list(df_baseline_clean)
print("Converting df_stress_train_clean to list")
stress_data_train_clean = utils.df_to_list(df_stress_clean)
...
# windows_b = utils.extract_windows(baseline_data, 0.25)
# print("baseline data windows : {}".format(len(windows_b)))
#
# # windows_s for stress data train
# windows_s = utils.extract_windows(stress_data, 0.25)
...
#     a_1 = Yax[ix]
#     a_2 = Zax[ix]
#     b = np.array([i for i in Xax[ix] if i < 25])
#     c = np.array([i for i in Yax[ix] if i < 200])
#     d = np.array([i for i in Zax[ix] if i < 10])
#     ax.scatter(d, b, c,
#             c=cdict[l], s=40,
